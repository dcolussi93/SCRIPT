...
& "$PSScriptRoot\pbi-GetMetadata.ps1" -tenantId $tenantId -userName $appId -password $Secure_String_Pwd  -path $path -debugMode $debugMode -servicePrincipalFlag $servicePrincipalFlag -workspacesListFlag $true -usersAccessFlag $true -dimListFlag $false -relListFlag $false 


### 2 - Workspaces, Users permissions, Reports, Datasets, Dashboards - CASO ADOLFO
#& "$PSScriptRoot\pbi-GetMetadata.ps1" -tenantId $tenantId -userName $appId -password $Secure_String_Pwd  -path $path -debugMode $debugMode -servicePrincipalFlag $servicePrincipalFlag -workspacesListFlag $true -dimListFlag $true -relListFlag $true -usersAccessFlag $true
...
#$relListFlag= $true
#$dimListFlag = $true
#& "$PSScriptRoot\pbi-GetMetadata.ps1" -tenantId $tenantId -userName $appId -password $Secure_String_Pwd  -path $path -debugMode $debugMode -servicePrincipalFlag $servicePrincipalFlag -workspacesListFlag $workspacesListFlag -dimListFlag $dimListFlag -relListFlag $relListFlag -usersAccessFlag $usersAccessFlag